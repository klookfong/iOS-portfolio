...
def films_by_cast_size
  ...
  execute(<<-SQL)
  select q.title, count(name) from (
    select * from movies
    join castings on movies.id = castings.movie_id 
    join actors ON castings.actor_id = actors.id
  ...
  group by q.title
  order by count(name) desc, q.title 
  SQL
end
...
def colleagues_of_garfunkel
  ...
  ) as q where q.name != 'Art Garfunkel' 
  SQL
end
